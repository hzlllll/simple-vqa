...
def create_image(filename, shape, color, number):
    r = randint(230, 255)
    g = randint(230, 255)
    b = randint(230, 255)
    ...
    draw_shape(draw, shape, color, number)
    del draw
...
def draw_shape(draw, shape, color, number):
    for i in range(number.value):
        if shape is Shape.RECTANGLE:
            ...
            draw.polygon([
                (x, y),
                (x + s * math.cos(TRIANGLE_ANGLE_1),
                 y + s * math.sin(TRIANGLE_ANGLE_1)),
                (x + s * math.cos(TRIANGLE_ANGLE_2),
                 y + s * math.sin(TRIANGLE_ANGLE_2)),
            ],
                         fill=color.value)
